...
def io_bound(param_name='filepath'):
    """
    Decorator for IO-bound operations.

    Args:
        param_name: Name of the parameter containing the filepath
    """
    ...
    def decorator(func):
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            if param_name in kwargs:
                filepath = kwargs[param_name]
            else:
                # Find the position of the filepath parameter
            ...
            return await resource_manager.submit_io_task(filepath, func, *args, **kwargs)
    ...
    def decorator(func):
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            def executor_run(*exec_args, **exec_kwargs):
                # Print process/thread info for debugging
                import os
                import threading
                process_id = os.getpid()
                thread_id = threading.get_ident()
                thread_name = threading.current_thread().name
                print(f"Executing in process {process_id}, thread {thread_id} ({thread_name})")
